CREATE TABLE Employee (
   ...
   Salary number(16,8) NOT NULL,
   Remarks varchar2(256)
 );
 
 CREATE TABLE Planet (
   PlanetID number(10) PRIMARY KEY NOT NULL,
   Name varchar2(256) NOT NULL,
   Coordinates number(20,8) NOT NULL
 );
 
 CREATE TABLE Shipment (
   ShipmentID number(10) PRIMARY KEY NOT NULL,
   ShipmentDate Date Null,
   Manager number(10) NOT NULL
     CONSTRAINT fk_Employee_EmployeeID REFERENCES Employee(EmployeeID),
   Planet number(10) NOT NULL
     CONSTRAINT fk_Planet_PlanetID1 REFERENCES Planet(PlanetID)
 );
 
 CREATE TABLE Has_Clearance (
   Employee number(10) NOT NULL
     CONSTRAINT fk_Employee_EmployeeID2 REFERENCES Employee(EmployeeID),
   Planet number(10) NOT NULL
     CONSTRAINT fk_Planet_PlanetID2 REFERENCES Planet(PlanetID),
   ClearanceLevel number(10) NOT NULL,
   PRIMARY KEY(Employee, Planet)
 );
 
 CREATE TABLE Client (
   AccountNumber number(10) PRIMARY KEY NOT NULL,
   Name varchar2(256) NOT NULL
 );
 
 CREATE TABLE Package (
   Shipment number(10) NOT NULL
     CONSTRAINT fk_Shipment_ShipmentID REFERENCES Shipment(ShipmentID),
   PackageNumber number(10) NOT NULL,
   Contents varchar2(256) NOT NULL,
   Weight number(20,8) NOT NULL,
...
INSERT INTO Shipment VALUES(1, to_date('3004/05/11', 'yyyy/mm/dd'), 1, 1);
INSERT INTO Shipment VALUES(2, to_date('3004/05/11', 'yyyy/mm/dd'), 1, 2);
